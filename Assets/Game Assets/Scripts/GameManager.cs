...
public class GameManager : MonoBehaviour
{
    ...
    public void TinhTocDo()
    {
        int currentLevel = GameManager.instance.levelNo;

        for (int i = 0; i < stackPos.transform.childCount; i++)
        {
            float baseSpeed = Constants.STARTING_BUILDING_BLOCK_SPEED;
            float stackBonus = increamentBlockSpeed * i;

            // Add difficulty: speed grows with level number
            float levelBonus = currentLevel * 0.5f; // tweak 0.5f to control difficulty growth

            stackPos.transform.GetChild(i)
                .GetComponent<BuildingBlockMoveSpeed>().buildingBlockMoveSpeed = baseSpeed + stackBonus + levelBonus;
        }
    }
